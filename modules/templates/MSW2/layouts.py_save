    ...
    if item.enabled and item.visible:
        ...
        if item.parent is not None:
            ...
            if item.parent.parent is None:
                ...
                toplevel = False
                # msw
                print 'aAAAAAAAAAAA item.module_type:', item.module_type
                if item.module_type == 10:
                    print "layouts.py in MSW2: my type is 10"
...
class S3OrgMenuLayout(S3NavigationItem):
    ...
    def layout(item):
        ...
        logo = IMG(_src="/%s/static/themes/MSW2/img/logo_small.png" %
                        current.request.application,
                   _alt="Sächsischer Flüchtlingsrat",
                   _width=40,
                   )
