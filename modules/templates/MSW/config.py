...
def config(settings):
    ...
    #msw
    settings.ui.pdf_logo = "eden/Msw/msw_schild_spinne.png"
    ...
    #
    # msw
    settings.security.policy = 2 # Organisation-ACLs
    ...
    def customise_org_organisation_controller(**attr):
        ...
        #table.year.comment = DIV(_class="tooltip",_title="%s|%s" % (T("Year Founded"), T("Year that the organization was founded, naturally")))
        return attr
    settings.customise_org_organisation_controller = customise_org_organisation_controller
    
    # deselect/select (does this work?) single fields
    def customise_org_organisation_resource(r, tablename):
        table = current.s3db.org_organisation
        table.website.readable  = False 
        table.website.writable  = False
        table.phone.readable  = False
        table.phone.writable  = False
        
        table.contact.readable  = True
        table.contact.writable  = True
    settings.customise_org_organisation_resource = customise_org_organisation_resource
    
  
    # msw: adding Asylumseeker to main menue (above)
    settings.modules["asylumseeker"] = Storage(
        name_nice=T("Asylumseeker"),
        module_type=2)

    # msw: adding residential to main menue (above)
